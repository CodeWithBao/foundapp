...
function SidebarContent({ onClose, user, handleLogout }) {
  ...
  return (
    <div className="flex flex-col h-full relative overflow-hidden">
      ...
      {/* Role Pill */}
      <div className="h-[46px] mx-[12px] my-[16px] px-[16px] rounded-[10px] bg-white/10 border border-white/10 text-white text-[13px] font-semibold flex items-center gap-2 shrink-0">
        <ShieldCheck className="w-[18px] h-[18px] text-white" />
        <span>Nhân viên Lost &amp; Found</span>
      </div>

      {/* Navigation */}
      <nav className="flex-1 px-[12px] py-2 space-y-2.5 overflow-y-auto z-10 relative">
        <p className="px-[12px] mb-[12px] text-[11px] font-bold text-white/50 uppercase tracking-widest">
          CHỨC NĂNG CHÍNH
        </p>
        {menuItems.map((item) => {
          const Icon = item.icon;
          const isActive = item.end
            ? location.pathname === item.to || location.pathname === '/staff/dashboard'
          ...
          return (
            <NavLink
              ...
              onClick={onClose}
              className={`flex items-center gap-[14px] px-[18px] h-[52px] rounded-[10px] text-[15px] transition-all duration-200 ${
                isActive
      ...
      <img
        ...
        alt=""
        aria-hidden="true"
        className="pointer-events-none absolute bottom-[125px] left-1/2 w-[118%] max-w-none -translate-x-1/2 object-contain opacity-[0.28] select-none z-0"
      />
      ...
      <img
        ...
        className="pointer-events-none absolute bottom-[70px] left-1/2 w-[145px] -translate-x-1/2 object-contain opacity-90 select-none z-0"
      />
        ...
        <div className="flex items-center gap-3.5 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-[14px] font-semibold text-white truncate leading-tight">
              {user?.fullName || user?.name || 'Trần Minh Tuấn'}
            </p>
            <p className="text-[12px] text-white/70 truncate leading-tight mt-[3px]">
              {user?.email || 'staff@dntu.edu.vn'}
            </p>
          </div>
        </div>
        <button
...
export default function StaffLayout() {
  ...
  const [sidebarOpen, setSidebarOpen] = useState(() => {
    const saved = localStorage.getItem('dntu_staff_sidebar_open');
  ...
  useEffect(() => {
    localStorage.setItem('dntu_staff_sidebar_open', sidebarOpen ? 'true' : 'false');
  }, [sidebarOpen]);
  ...
  return (
    <div className="min-h-screen flex bg-[#F8F6F3]">
      ...
      >
          ...
          <div className="relative z-10 flex h-full items-center justify-between px-4 sm:px-6">
            ...
            <div className="relative z-10 flex items-center gap-3">
              <button
                ...
                aria-label={sidebarOpen || mobileDrawerOpen ? "Đóng menu Staff" : "Mở menu Staff"}
                title="Đóng / Mở menu Staff"
        ...
        <main className="flex-1 bg-[#F8F6F3] p-4 sm:p-6 lg:px-6 py-5 min-h-[calc(100vh-72px)] pb-[110px] max-w-[1380px] mx-auto w-full">
